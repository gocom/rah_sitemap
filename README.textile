...
h3. Number of URLs per sitemap

How many URLs a single rendered sitemap will contain. If all items, such as articles, do not fit into a single sitemap, they are split into multiple separate individual sitemaps, that are then included in the rendered sitemap index listing. Default limit is 50000, which is the Sitemaps.org spec's maximum allowed limit.

h3. Number of records buffered from database at once

How many rows are buffered from the database at once. Rows are buffered in chunks to limit memory usage, working around underlying system limitations. Textpattern CMS unfortunately does not fully support unbuffered queries, and as such we can not iterate rows by-row, but are required to buffered rows in chunks to mitigate greedy loading limitations. The lower the number, the less rows are buffered at once, lowering peak memory usage, but also slowing sitemap rendering due to increased number of executed queries. Default limit is 1000.
...
h3. Version 4.0.4 - 2026/09/13

* Fixed: Handle edge cases where an user erroneously configures a higher buffer chunk size than the total sitemap URL limit.
* Fixed: Explicitly define @$urlset@ variable in when rendering the site variant of the sitemaps. Even in development mode, during sitemap rendering the notice about undefined variable was silenced by output buffer clearing; Now it the source itself has been mitigated.
* Updated development environment, implemented static analysis tests and initial possibility for unit tests. New PHP 8.5 based runtime for running the test suite.

h3. Version 4.0.3 - 2026/09/11

* Fixed: reduced peak memory usage, especially when rendering sitemaps for sites' with thousands of articles containing a lot of textual content and fields. Thank you, "revdazza":https://github.com/revdazza.
* Rows are now buffered in chunks from Textpattern's database. The chunk size is configurable in the plugin's preferences with **Number of records buffered from database at once** settings.
* Only the bare minimum necessary columns are loaded from the database when rendering article sitemap; Only article fields expected by Textpattern's core URL builder are loaded, normalizing the fields to a consistent set and reducing the buffer's memory usage.
